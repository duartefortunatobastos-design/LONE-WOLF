(function () {
    ...
    function initCountUp() {
        ...
        function parseTime(value) {
            ...
            if (parts.length === 3) {
                return { seconds: parts[0] * 3600 + parts[1] * 60 + parts[2], parts: 3 };
            }
            return { seconds: parts[0] * 60 + (parts[1] || 0), parts: 2 };
        }

        function formatTime(totalSeconds, parts) {
        ...
        function setFinal(element) {
            const type = element.dataset.countType || "number";
            const suffix = element.dataset.countSuffix || "";
            if (type === "time") {
                element.textContent = element.dataset.countValue || element.textContent;
                return;
            }
            element.textContent = `${element.dataset.countValue || "0"}${suffix}`;
        }
        ...
        function animate(element) {
            ...
            if (type === "time") {
                const parsed = parseTime(element.dataset.countValue || "0:00");
                const tick = (now) => {
                    const progress = Math.min((now - start) / duration, 1);
                    const eased = 1 - Math.pow(1 - progress, 3);
                    const current = Math.round(parsed.seconds * eased);
                    element.textContent = formatTime(current, parsed.parts);
                    if (progress < 1) {
            ...
            const target = parseFloat(element.dataset.countValue || "0");
            const tick = (now) => {
                const progress = Math.min((now - start) / duration, 1);
                const eased = 1 - Math.pow(1 - progress, 3);
    ...
    function initRaceFilters() {
        ...
        filters.addEventListener("click", (event) => {
            ...
            filters.querySelectorAll(".races-filter").forEach((item) => item.classList.remove("is-active"));
            button.classList.add("is-active");

            const key = button.dataset.filter;
            cards.forEach((card) => {
                const show = key === "all" || card.dataset.distancia === key || card.dataset.ano === key;
