...
function obter_ou_criar_conversa(mysqli $conn, ?int $userId, string $nome, string $email): int
{
    ...
    if ($conversa) {
        ...
        if ($userId) {
            ...
            $stmt = $conn->prepare("UPDATE conversas SET nome = ? WHERE id = ?");
            $stmt->bind_param("si", $nome, $conversaId);
            $stmt->execute();
            $stmt->close();
        }
    ...
    if ($userId) {
        ...
        $stmt = $conn->prepare("INSERT INTO conversas (user_id, nome, email) VALUES (NULL, ?, ?)");
...
function email_esta_bloqueado(mysqli $conn, string $email): bool
{
    $stmt = $conn->prepare("SELECT bloqueado FROM utilizadores WHERE email = ? LIMIT 1");
    $stmt->bind_param("s", $email);
    $stmt->execute();
    $resultado = $stmt->get_result();
    $utilizador = $resultado->fetch_assoc();
